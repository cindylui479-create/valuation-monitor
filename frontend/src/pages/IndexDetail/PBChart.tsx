...
import type { QuotePoint } from "@/types/api";

interface Props {
  quotes: QuotePoint[];
}

export default function PBChart({ quotes }: Props) {
  const option = useMemo(() => {
    const dates = quotes.map((q) => q.date);
    const pbs = quotes.map((q) => (q.pb == null ? null : parseFloat(q.pb)));
    const validPbs = pbs.filter((v): v is number => v != null).sort((a, b) => a - b);
    const quantile = (q: number): number | null => {
      if (validPbs.length === 0) return null;
    ...
    ].filter((r): r is { y: number; label: string; color: string } => r.y != null);
