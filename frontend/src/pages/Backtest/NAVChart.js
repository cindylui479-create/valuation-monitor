...
export default function NAVChart({ strategies }) {
    const [yMode, setYMode] = useState("pct");
    const option = useMemo(() => {
        const active = strategies.filter((s) => s != null);
        const longest = active.reduce((a, b) => (b.nav_curve.length > (a?.nav_curve.length ?? 0) ? b : a), null);
        const dates = longest?.nav_curve.map((p) => p.date) ?? [];
        const transform = (nav, baseNav) => {
            if (yMode === "nav")
                return nav;
            if (yMode === "pct")
        ...
        const yAxisFormatter = yMode === "pct" ? (v) => `${v >= 0 ? "+" : ""}${v.toFixed(0)}%`
            : yMode === "log" ? (v) => Math.exp(v).toFixed(2)
                : undefined;
        return {
            title: {
    ...
    return (_jsxs("div", { children: [_jsx("div", { style: { display: "flex", justifyContent: "flex-end", gap: 4, marginBottom: 4 }, children: _jsx("div", { style: {
                        display: "inline-flex", border: "1px solid #d1d5db", borderRadius: 4, overflow: "hidden",
                    }, children: ["pct", "nav", "log"].map((m) => (_jsx("button", { type: "button", onClick: () => setYMode(m), style: {
                            ...
                            cursor: "pointer",
                        }, children: m === "pct" ? "相对涨跌 %" : m === "nav" ? "NAV 绝对" : "对数 Y" }, m))) }) }), _jsx(ReactECharts, { option: option, style: { height: 380, width: "100%" } })] }));
}
