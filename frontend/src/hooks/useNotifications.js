    ...
    useEffect(() => {
        ...
        const poll = async () => {
            try {
                ...
                // HIGH 跳变新增
                const newTrans = trans.items.filter((t) => t.severity === "HIGH" && !seenTransitionsRef.current.has(`${t.entity_code}@${t.date}`));
                for (const t of newTrans.slice(0, 3)) {
                    new Notification(`档位跳变 · ${t.entity_name} (${t.entity_code})`, {
                ...
                seenTransitionsRef.current = newTransKeys;
                // 极度低估新增
                const newLow = opps.low_valuations.filter((o) => o.tier === "极度低估" && !seenLowRef.current.has(o.entity_code));
                for (const o of newLow.slice(0, 3)) {
                    new Notification(`极度低估 · ${o.entity_name} (${o.entity_code})`, {
